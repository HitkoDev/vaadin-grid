...
vaadin.elements.grid.ColumnBaseBehavior = {
  ...
  created: function() {
    ...
    this._templateObserver = Element ?
      new FlattenedNodesObserver(this, handler) : dom(this).observeNodes(handler);
  },
  ...
  _selectFirstTemplate: function(selector) {
    return Array.prototype.filter.call(dom(this).querySelectorAll(selector), function(el) {
      return dom(el).parentNode === this;
    }.bind(this))[0];
  },
...
vaadin.elements.grid.ColumnBehaviorImpl = {
  ...
  _templateChanged: function(template) {
    ...
    if (template && template.templatizer) {
      dom(this.root).appendChild(template.templatizer);
    }
