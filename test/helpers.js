import { DomApi, dom } from '../../@polymer/lib/legacy/polymer.dom.js';
...
function getRows(container) {
  return Array.prototype.slice.call(dom(container).querySelectorAll('.vaadin-grid-row'));
}

function getRowCells(row) {
  return Array.prototype.slice.call(dom(row).querySelectorAll('.vaadin-grid-cell'));
}

function getCellContent(cell) {
  return cell ? cell.getContentChildren(Element ? 'slot' : 'content')[0] : null;
}
...
function flushColumns(grid) {
  Array.prototype
    .forEach.call(grid.querySelectorAll('vaadin-grid-column'),
    function(col) {
