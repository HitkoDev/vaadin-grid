...
$_documentContainer.innerHTML = `<dom-module id="vaadin-grid-navigation-themability-styles">
  <template>
    <style>
      :host(:focus),
      #table:focus {
...
vaadin.elements.grid.TableKeyboardBehaviorImpl = {
  ...
  _onFooterFocus: function(e) {
    ...
    this.interacting = false;
    this._virtualFocus = this._virtualFocus ||
  ...
  _onPageDown: function(e) {
    ...
    if (this._virtualFocus === this.$.items) {
      ...
      this._virtualFocus._focusedRowIndex += (this.$.scroller.lastVisibleIndex - prevLastVisible) ||
       (this.$.scroller.lastVisibleIndex - this._virtualFocus._focusedRowIndex);
      this._ensureVirtualFocusInViewport();
    } else {
  ...
  _onPageUp: function(e) {
    ...
    if (this._virtualFocus === this.$.items) {
      ...
      this._scrollPageUp();
      this._virtualFocus._focusedRowIndex -= (prevLastVisibleIndex - this.$.scroller.lastVisibleIndex) ||
  ...
  _onSpace: function(e) {
    ...
    var firstElementChild = focusedCell.getContentChildren(Element ? 'slot' : 'content')[0].firstElementChild;
  ...
  _onCellFocus: function(e) {
    ...
    var rowIndex = Array.prototype.indexOf.call(dom(container).children, row);
    if (container === this.$.items) {
      rowIndex = row.index;
    }

    container._focusedRowIndex = rowIndex;
    container._focusedCellIndex = Array.prototype.indexOf.call(dom(row).children, cell);
  ...
  _ensureVirtualFocusInViewport: function() {
    ...
    if (this._virtualFocus === this.$.items && ((focusedIndex < scaledVirtualStart) ||
        (focusedIndex > scaledVirtualStart + this.$.scroller._physicalCount))) {
      this.$.scroller.scrollToScaledIndex(focusedIndex);
...
*/
vaadin.elements.grid.TableKeyboardBehavior = [vaadin.elements.grid.TableKeyboardBehaviorImpl, IronA11yKeysBehavior];
