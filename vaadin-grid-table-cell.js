...
var vaadinGridTableCellBehavior = {
  ...
  _toggleContent: function(isAttached, cellContent, insertionPoint, scrollerInsertionPoint) {
    ...
    if (isAttached) {
      if (dom(cellContent).parentNode !== this.target) {
        dom(this.target).appendChild(cellContent);
      }
      dom(this).appendChild(insertionPoint);
    } else {
      this.async(function() {
        if (!this.isAttached && dom(cellContent).parentNode === this.target) {
          dom(this.target).removeChild(cellContent);
        }
      });
    }
  },
  ...
  _instanceChanged: function(instance, target) {
    ...
    dom(this._cellContent).appendChild(this.instance.root);

    // `aria-labelledby` a11y attribute in the focus trap needs cell IDs
...
Polymer({
  ...
  observers: [
    ...
    '_isEmptyChanged(_isEmpty, isAttached)',
  ...
  _contentFromPoint: function(x, y) {
    if (Settings.useShadow) {
      ...
      if (cell && cell.getContentChildren) {
        return cell.getContentChildren(Element ? 'slot' : 'content')[0];
      }
    } else {
  ...
  _resizableChanged: function(resizable) {
    if (resizable) {
      ...
      dom(this).appendChild(this._resizeHandle);
    } else if (this._resizeHandle) {
      this.unlisten(this._resizeHandle, 'track', '_onTrack');
      dom(this).removeChild(this._resizeHandle);
    }
  },
  ...
  _onTrack: function(e) {
    ...
    Array.prototype.slice.call(dom(header).querySelectorAll('.vaadin-grid-row:last-child .vaadin-grid-cell'))
    .sort(
      function(a, b) {
  ...
  _getHeaderCellByColumn: function(column) {
    var header = this.parentElement.parentElement;
    return Array.prototype.filter.call(dom(header)
      .querySelectorAll('.vaadin-grid-row:last-child .vaadin-grid-cell'), function(cell) {
