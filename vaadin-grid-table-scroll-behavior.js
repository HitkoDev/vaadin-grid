...
$_documentContainer.innerHTML = `<dom-module id="vaadin-grid-table-table-scroll-styles">
  <template>
    <style>
      #table {
        position: relative;
...
vaadin.elements.grid.TableScrollBehaviorImpl = {
  ...
  _onWheel: function(e) {
    ...
    if (e.deltaMode === 1) {
      ...
      deltaY *= grid.$.scroller._physicalAverage;
    }
  ...
  _frozenCellsChanged: function() {
    this.debounce('cache-elements', function() {
      dom(this.domHost.root).querySelectorAll('.vaadin-grid-cell').forEach(function(cell) {
        cell.style.transform = '';
      });
      this._frozenCells = Array.prototype.slice.call(dom(this.domHost.root).querySelectorAll('[frozen]'));
      this._translateStationaryElements();
    });
