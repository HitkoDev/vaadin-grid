...
$_documentContainer.innerHTML = `<dom-module id="vaadin-grid-column-reordering-themability-styles">
  <template>
    <style>
...
vaadin.elements.grid.TableColumnReorderingBehavior = {
  ...
  _setSiblingsReorderStatus: function(column, status) {
    Array.prototype.filter.call(dom(dom(column).parentNode).children, function(child) {
  ...
  _onDragOver: function(e) {
    ...
    var path = Element ? e.composedPath() : dom(e).path;
  ...
  _isSwappableByPosition: function(targetColumn, clientX) {
    var targetCell = Array.prototype.filter.call(dom(this.$.header).querySelectorAll('.vaadin-grid-cell'), function(cell) {
  ...
  _getCellByCellContent: function(cellContent) {
    if (Element) {
      ...
      var insertionPoint = dom(cellContent).getDestinationInsertionPoints()[0];
      return dom(insertionPoint).parentNode;
    }
  },
