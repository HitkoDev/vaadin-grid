...
import './vaadin-grid-table-outer-scroller.js';
import './vaadin-grid-table-header-footer.js';
import './vaadin-grid-table-cell.js';
import './vaadin-grid-table-focus-trap.js';
import './vaadin-grid-table-row.js';
...
$_documentContainer.innerHTML = `<dom-module id="vaadin-grid-table-styles">
  <template>
    <style>
...
`,
  ...
  _columnTreeChanged: function(columnTree, physicalItems, count) {
    ...
    afterNextRender(this, this._update);
    ...
    // make sure cells have been created inside rows before appending.
    flush(this);
    this._updateLastColumn();
  },

  _updateLastColumn: function() {
    Array.prototype.forEach.call(dom(this.domHost.root).querySelectorAll('.vaadin-grid-row'), function(row) {
  ...
  _updateHeaderFooterMetrics: function() {
    ...
    afterNextRender(this.$.header, function() {
      this._updateHeaderFooterMetricsSync();
  ...
  _createPool: function(size) {
    ...
    for (var i = 0; i < size; i++) {
      ...
      dom(this.$.items).appendChild(row);
    }
